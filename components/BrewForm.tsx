...
export interface BrewFormProps {
  initialMethod?: BrewMethod;
  /** Hide the method picker (edit mode: the method of a logged brew is fixed). */
  lockMethod?: boolean;
  initialBeanId?: number | null;
  initialGrinderId?: number | null;
  ...
  banner?: string;
  primaryLabel: string;
  onPrimary: (draft: BrewDraft) => Promise<void> | void;
  secondaryLabel?: string;
  onSecondary?: (draft: BrewDraft) => Promise<void> | void;
}
...
export function BrewForm({
  ...
  useEffect(() => {
    ...
    if (initialParams && method === initialMethod) {
      reset({ ...defaults, ...initialParams });
    } else {
      reset(defaults);
    }
  }, [method, initialMethod, initialParams, reset]);

  function buildDraft(values: FormValues): BrewDraft {
  ...
  function submitWith(handler: (draft: BrewDraft) => Promise<void> | void) {
    return handleSubmit(async (values: FormValues) => {
      setSaving(true);
      try {
        await handler(buildDraft(values));
      } catch (err) {
  ...
  return (
    <ScrollView style={styles.scroll} contentContainerStyle={styles.content}>
      ...
      {banner != null && method === initialMethod && (
        <View style={styles.suggestionBanner}>
          <Text style={styles.suggestionBannerText}>{banner}</Text>
        </View>
      )}
      ...
      {secondaryLabel != null && onSecondary != null && (
        <Pressable
          onPress={submitWith(onSecondary)}
          disabled={saving}
